...
export async function listUnreadNotifications(profileId, limit = 10) {
  ...
  try {
    const { data: rows, error } = await supabase
      .from('notifications')
      .select('id, conversation_id, message_id, notification_type, title, body, pre_registration_id, expiry_kind, resource_id, student_id, lesson_module_id, is_read, created_at')
      .eq('profile_id', profileId)
      .eq('is_read', false)
